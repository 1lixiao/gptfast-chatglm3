...
def get_group_qparams(w, n_bit=4, groupsize=128):
    ...
    # print(w.shape) torch.Size([4608, 4096])
    # print(to_quant.shape) torch.Size([589824, 32])
    assert torch.isnan(to_quant).sum() == 0

    max_val = to_quant.amax(dim=1, keepdim=True)
    # print(max_val.shape)  torch.Size([589824, 1])
    min_val = to_quant.amin(dim=1, keepdim=True)
    # print(min_val.shape)  torch.Size([589824, 1])
    max_int = 2 ** n_bit - 1
    # print(max_int)  15
    scales = (max_val - min_val).clamp(min=1e-6) / max_int
    # print(scales.shape) # torch.Size([524288, 1])

    zeros = min_val + scales * (2 ** (n_bit - 1))
    #  print(zeros.shape) torch.Size([524288, 1])
    #  print(scales.to(torch.bfloat16).reshape(w.shape[0], -1).shape) torch.Size([4096, 128])
...
class GPTQQuantHandler(QuantHandler):
    ...
    def create_quantized_state_dict(
        ...
        inputs = GPTQQuantHandler.get_inputs(self.mod, tokenizer, calibration_tasks, calibration_limit,
...
def replace_linear_weight_only_int8_per_channel(module):
    for name, child in module.named_children():
        if isinstance(child, nn.Linear):
            bias = ['query_key_value'].__contains__(name)
            setattr(module, name, WeightOnlyInt8Linear(child.in_features, child.out_features, bias=bias))
        ...
        if isinstance(child, nn.Linear):
            if _check_linear_int4_k(child.in_features, groupsize, inner_k_tiles):
                ...
                setattr(module, name, WeightOnlyInt4Linear(
                    child.in_features, child.out_features, bias=bias,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', '-q', type=str, default='int8', choices=['int8', 'int4', 'int4-gptq'],
    ...
    parser.add_argument('--calibration_tasks', type=str, nargs='+', default=['wikitext'],
                        help='tasks to do gptq calibration on, if doing gptq')
    parser.add_argument('--calibration_limit', type=int, default=1000,
                        help='number of samples to use for gptq calibration')
    parser.add_argument('--calibration_seq_length', type=int, default=100,
                        help='length of sequences to use for gptq calibration')
    parser.add_argument('--pad_calibration_inputs', type=bool, default=False,
    ...
    quantize(args.checkpoint_path, args.mode, args.groupsize, args.calibration_tasks, args.calibration_limit,
             args.calibration_seq_length, args.pad_calibration_inputs, args.percdamp, args.blocksize, args.label)
